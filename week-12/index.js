...
function renderTable() {
    ...
    const rows = data.map(({ id, first_name, last_name, email, gender, ip_address, contact_number, city, state, postal_code, university }) => table.appendChild(
        htmlToElement(
            `<tr id="table-row-${id}">
            <td>${id}</td>
            <td>${first_name}</td>
...
const resetTable = () => {
    ...
    const inputControl = document.getElementById(`input-term`);
    inputControl.value = "";
    window.location = 'http://127.0.0.1:5500/week-12/index.html'
